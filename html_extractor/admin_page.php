...
<div class="divSearchBar">
    ...
    <div>
        ...
        <center><div class="spinner-border text-primary loading" id="loading" style="display:none;" role="status">
...
while ($files->valid()) {
    ...
    if ($valid and ($files->getDepth() - 1 == $deep or $files->getDepth() == $deep)) {
        ?>
        <ion-icon style="display:none" onClick='deleteDir("<?php echo $file;?>")' name='close-circle-outline'></ion-icon><?php echo $indent;?>├ <a href='<?php echo $file;?>'> <ion-icon name='folder-outline'></ion-icon><?php echo $filename;?></a><br>
        <?php
        ...
        ?>
        <ion-icon style="display:none" onClick='deleteDir("<?php echo $file;?>")' name='close-circle-outline'></ion-icon><?php echo $indent;?>└ <a href='<?php echo $file;?>' style='background-color:#b5f3fd;'> <ion-icon name='folder-outline'></ion-icon><?php echo $filename;?></a><br>
        <?php
...
<center>
    <div style="display: inline-block;margin-left:-16%;"class="divSearchBar">
        <h5>Il y a un total de <?php echo $nbFilesIndexed; if($nbFilesIndexed == 1){echo " document indexé.";}else{echo " documents indexées.";}?></h5>
        </br>
...
<div class="divSearchBar">
    ...
    <div class="divContainer">
        <form action="admin_page.php" method="POST" enctype="multipart/form-data">
            ...
            <table>
            <tr><td><label>Votre fichier: </label></td><td><input id="fileData" type="file" name="fileData[]" multiple></td></tr>
            

            <!-- <label>Votre dossier: </label><button onclick="document.getElementById('folderData').click()">Selectionnez un dossier</button> -->
            <tr><td><label>Votre dossier: </label></td><td><input id="folderData" type="file" name="fileData[]" multiple directory="" webkitdirectory="" moxdirectory=""></td></tr>


            <tr><td><label>Chemin des fichiers: </label></td><td><input type="text" name="newPath" placeholder="Racine si vide"></td></tr>
            </table>
